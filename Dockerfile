...
# ADD a directory called Hello-world-docker-java inside the UBUNTU IMAGE where you will be moving all of these files under this
# DIRECTORY to
ADD . /usr/local/Hello-world-docker-java
...
RUN cd /usr/local/Hello-world-docker-java && mvn assembly:assembly
...
CMD ["java", "-cp", "/usr/local/Hello-world-docker-java/target/Hello-world-docker-java-0.0.1-SNAPSHOT-jar-with-dependencies.jar", "org.docker.java.HelloWorldPing"]